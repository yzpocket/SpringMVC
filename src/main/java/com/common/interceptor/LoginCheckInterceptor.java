...
public class LoginCheckInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest req, HttpServletResponse res, Object handler) throws Exception{
		...
		if(user==null) {
			...
			if("true".equals(header)) {
				res.sendError(400); //ajax요청이라면 400에러를 발생시키자.==>error 처리하는 콜백함수에서 받아 처리
			}else {
			req.setAttribute("message", "로그인 해야 이용 가능합니다.");
	...
	//[2] Contoller를 실행한 후, 아직 뷰를 실행하기 전에 호출되는 메서드 ModelAndView mv가 더들어감.
	@Override
	...
	//[3] Controller를 실행하고 뷰를 실행한 후 호출되는 메서드 
	@Override
